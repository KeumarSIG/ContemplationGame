...
public class BirdNew : MonoBehaviour 
{
	...
	IEnumerator Flying()
	{
		...
		{
			Moving(_DirectionOfCurrentDestination);

			// If the bird is about to collide with something
		...
		// The bird has to avoid
		if (m_IsChangingDirection == true)
		{
			StartCoroutine(AvoidingUp());
		}
	...
	void Moving(Vector3 DirectionOfCurrentDestination)
	{
		ChangeOrientation(DirectionOfCurrentDestination);
		GoForward(DirectionOfCurrentDestination);
		CheckCollision();
	}
	...
	IEnumerator AvoidingUp()
	{
		StartCoroutine(ResetChangeDirection());

		Vector3 _DirectionOfCurrentDestination = m_CurrentDestination - transform.position;
		_DirectionOfCurrentDestination = new Vector3(_DirectionOfCurrentDestination.x, transform.position.y, _DirectionOfCurrentDestination.z);
	...
	void GoUpwards()
	{
		m_Rb.AddForce(Vector3.up * m_SpdTakeOff * 2);
	}
